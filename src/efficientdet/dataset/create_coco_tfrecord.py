...
flags.DEFINE_integer('num_threads', 1, 'Number of threads to run.')
FLAGS = flags.FLAGS
...
def _load_object_annotations(object_annotations_file):
  ...
  logger.info('Building bounding box index.')
  ...
  logger.info('%d images are missing bboxes.', missing_annotation_count)
...
def _load_caption_annotations(caption_annotations_file):
  ...
  logger.info('Building caption index.')
  ...
  logger.info('%d images are missing captions.', missing_annotation_count)
...
def _create_tf_record_from_coco_annotations(image_info_file,
  ...
  for idx, (_, tf_example, num_annotations_skipped) in enumerate(
      pool.imap(
          ...
          [(image, image_dir, _get_object_annotation(image['id']),
           ...
           for image in images])):
    if idx % 100 == 0:
      logger.info('On image %d of %d', idx, len(images))
  ...
  logger.info('Finished writing, skipped %d annotations.',
